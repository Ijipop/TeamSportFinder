...
const DashboardPlayerPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = formData.id
      ? `${baseUrl}/api/player/profile/${formData.id}/`
      : `${baseUrl}/api/player/profile/`;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSnackbarMessage("Erreur : " + (errorData.error || res.statusText));
        setSnackbarSeverity("error");
        setSnackbarOpen(true);
      }
    } catch (err) {
      setSnackbarMessage("Erreur réseau : " + err);
      setSnackbarSeverity("error");
      setSnackbarOpen(true);
    }
  };
